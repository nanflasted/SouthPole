...
public class SouthPoleUtil {
    ...
    public static Tile[][] generateWorld(int size) {
        ...
        double distFromCenterToOcean = (size / 2) - WATER_BORDER_SIZE;

        //generate the world.
        for (int x = 0; x != size; x++) {
            System.out.println("");
            for (int y = WATER_BORDER_SIZE; y != (size - WATER_BORDER_SIZE); y++) {
                //100 % light snow at border, 100% heavy snow at center.
                double distFromCenter = Math.sqrt(Math.pow((x - size), 2) + Math.pow((y - size), 2));
